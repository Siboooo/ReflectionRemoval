...
def generator(input, is_train, reuse = False, sample = False):
    if sample:
        BATCH_SIZE = 1
    else:
        BATCH_SIZE = 4
    ...
    with tf.variable_scope("gen", reuse = reuse):
        #attention maps
        mask = np.ones((BATCH_SIZE, IMAGE_HIGHT, IMAGE_WIDTH, 1))
        h = np.zeros((BATCH_SIZE, IMAGE_HIGHT, IMAGE_WIDTH, 1))
        c = np.zeros((BATCH_SIZE, IMAGE_HIGHT, IMAGE_WIDTH, 1))

        mask_list = []
        for iter in range(4):
            x = tf.concat([input, mask], 3)
        ...
        #Block 1
        conv = tf.layers.conv2d(x, 64, kernel_size=[9, 9], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        ...
        #frame1
        conv = tf.layers.conv2d_transpose(act, 3, kernel_size=[3, 3], padding="same", strides=[2, 2])
...
def attentionGenerator(input, is_train, reuse = False):
    with tf.variable_scope("gen", reuse = reuse):
...
def encoderGenerator(input, is_train, reuse = False):
    with tf.variable_scope("gen", reuse = reuse):
        #Block 1
...
def gradientPrediction(input, is_train, reuse = False):
    with tf.variable_scope("edge", reuse = reuse):
        ...
        act = tf.nn.relu(conv)
        conv = tf.layers.conv2d(act, 512, kernel_size=[4, 4], padding="same", strides=[2, 2])
        act = tf.nn.relu(conv)
        res4 = act
        ...
        conv = tf.layers.conv2d(act, 1024, kernel_size=[7, 7], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        conv = tf.layers.conv2d(act, 512, kernel_size=[1, 1], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)

        conv = tf.layers.conv2d(act, 256, kernel_size=[3, 3], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        deconv = tf.layers.conv2d_transpose(act, 256, kernel_size=[4, 4], padding="same", strides=[2, 2])
        act = tf.nn.relu(deconv)
        cat = tf.concat([act, res4], 1)

        conv = tf.layers.conv2d(cat, 128, kernel_size=[3, 3], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        deconv = tf.layers.conv2d_transpose(act, 128, kernel_size=[4, 4], padding="same", strides=[2, 2])
        act = tf.nn.relu(deconv)
        cat = tf.concat([act, res3], 1)

        conv = tf.layers.conv2d(cat, 64, kernel_size=[3, 3], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        deconv = tf.layers.conv2d_transpose(act, 64, kernel_size=[4, 4], padding="same", strides=[2, 2])
        act = tf.nn.relu(deconv)
        cat = tf.concat([act, res2], 1)

        conv = tf.layers.conv2d(cat, 32, kernel_size=[3, 3], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        deconv = tf.layers.conv2d_transpose(act, 32, kernel_size=[4, 4], padding="same", strides=[2, 2])
        act = tf.nn.relu(deconv)
        cat = tf.concat([act, res1], 1)

        conv = tf.layers.conv2d(cat, 64, kernel_size=[4, 4], padding="same", strides=[1, 1])
        act = tf.nn.relu(conv)
        conv = tf.layers.conv2d(act, 1, kernel_size=[5, 5], padding="same", strides=[2, 2])
        result = tf.nn.tanh(conv)
...
def discriminator(input, is_train, reuse = False):
    with tf.variable_scope("dis", reuse = reuse):
        ...
        result = tf.layers.dense(act, 1, activation='sigmoid')
    return result
...
def discriminator3(input, is_train, reuse=False):
    with tf.variable_scope("dis") as scope:
        if reuse:
            scope.reuse_variables()
