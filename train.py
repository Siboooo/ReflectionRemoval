...
def train2():
    ...
    dis_result = discriminator(generated_image, d_is_train)
    real_result = discriminator(real_image, d_is_train)
    #generated_result = discriminator(generated_image, d_is_train, reuse = True)

    #d_loss = wasserstein_loss(target_result, dis_result)
    d_loss = descriminator_loss(real_result, dis_result)
    ...
    for epoch in range(EPOCH):
        ...
        for batch in range(int(x_train.shape[0]/BATCH_SIZE)):
            ...
            for iter in range(1):
                ...
                '''
                _, dLoss = sess.run([trainer_d, d_loss],
                    feed_dict={input_image: x_batch, real_image: y_batch, d_is_train: True})
                des_loss.append(dLoss)
